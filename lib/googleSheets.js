...
function getGoogleSheetsClient() {
  ...
  if (!serviceAccountEmail || !privateKey) {
    throw new Error('Google Sheets credentials not configured. Set GOOGLE_SERVICE_ACCOUNT_EMAIL and GOOGLE_PRIVATE_KEY in environment.');
  }
 ...
 */
export async function readKinoaSheet(sheetUrl) {
 ...
 */
export async function updateKinoaSheetHeader(sheetUrl, projectInfo) {
  const sheets = getGoogleSheetsClient();
  const spreadsheetId = extractSpreadsheetId(sheetUrl);
 ...
 */
export async function appendLogsToKinoaSheet(sheetUrl, logs) {
  const sheets = getGoogleSheetsClient();
  const spreadsheetId = extractSpreadsheetId(sheetUrl);
  ...
  const { nextDataRow, totalRowCount, sheetId } = await readKinoaSheet(sheetUrl);
 ...
 */
export async function syncToKinoaSheet(sheetUrl, projectInfo, logs) {
  // Update header metadata
  await updateKinoaSheetHeader(sheetUrl, projectInfo);
  
  // Append logs
  const result = await appendLogsToKinoaSheet(sheetUrl, logs);
